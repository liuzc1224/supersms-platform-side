<template>
  <Modal v-model="showModal" :title="$t('cardManagement.import')" :mask-closable="false" width="500px">
    <div slot="close">
...
<script>
  import {importCard} from "@/api/operation-config"
  export default {
    name: "import-modal",
    props: ['editModal'],
    data(){
    ...
    methods:{
      ...
      handleSubmit(){
        if(this.file){
          ...
          submitData.append("cardNo",this.file);
          importCard( submitData ).then(res => {
            this.loading=false;
            if(res.data.success){
